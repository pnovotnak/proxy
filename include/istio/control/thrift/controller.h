...
#ifndef ISTIO_CONTROL_THRIFT_CONTROLLER_H
#define ISTIO_CONTROL_THRIFT_CONTROLLER_H

#include "include/istio/control/thrift/request_handler.h"
...
// It takes ThriftClientConfig and performs tasks to enforce
// mixer control over THRIFT requests.
class Controller {
 public:
  virtual ~Controller() {}

  // Creates a THRIFT request handler.
  ...
  struct Options {
    Options(const ::istio::mixer::v1::config::client::ThriftClientConfig& config,
            const ::istio::utils::LocalNode& local_node)
        : config(config), local_node(local_node) {}

    // Mixer filter config
    const ::istio::mixer::v1::config::client::ThriftClientConfig& config;
...
#endif  // ISTIO_CONTROL_THRIFT_CONTROLLER_H
