...
#include "src/istio/control/thrift/attributes_builder.h"
...
void ServiceContext::BuildParsers() {
  ...
  for (const auto &api_spec : service_config_->thrift_api_spec()) {
    api_spec_.MergeFrom(api_spec);
  }
  api_spec_parser_ = ::istio::api_spec::ThriftApiSpecParser::Create(api_spec_);
...
void ServiceContext::AddApiAttributes(
  ...
  std::string path;
  if (check_data->FindHeaderByType(CheckData::HEADER_METHOD, &thrift_method) &&
      check_data->FindHeaderByType(CheckData::HEADER_PATH, &path)) {
    api_spec_parser_->AddAttributes(thrift_method, path, attributes);
  }
