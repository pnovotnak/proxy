...
#include "src/envoy/thrift/mixer/control_factory.h"
#include "src/envoy/thrift/mixer/filter.h"

using ::istio::mixer::v1::config::client::ThriftClientConfig;
...
class FilterFactory : public NamedNetworkFilterConfigFactory {
 public:
  ...
  Network::FilterFactoryCb createFilterFactoryFromProto(
    ...
    return createFilterFactory(dynamic_cast<const ThriftClientConfig&>(config),
                               context);
  }

  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return ProtobufTypes::MessagePtr{new ThriftClientConfig};
  }

  std::string name() override { return "mixer"; }

 private:
  Network::FilterFactoryCb createFilterFactory(const ThriftClientConfig& config_pb,
                                               FactoryContext& context) {
    std::unique_ptr<Thrift::Mixer::Config> config_obj(
        new Thrift::Mixer::Config(config_pb));

    auto control_factory = std::make_shared<Thrift::Mixer::ControlFactory>(
        std::move(config_obj), context);
    return [control_factory](Network::FilterManager& filter_manager) -> void {
      std::shared_ptr<Thrift::Mixer::Filter> instance =
          std::make_shared<Thrift::Mixer::Filter>(control_factory->control());
