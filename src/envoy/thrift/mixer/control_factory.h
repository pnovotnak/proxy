...
// Envoy stats perfix for Thrift filter stats.
const std::string kThriftStatsPrefix("thrift_mixer_filter.");
 ...
 public:
  ControlFactory(std::unique_ptr<Config> config,
      ...
      : control_data_(std::make_shared<ControlData>(
            std::move(config), generateStats(kThriftStatsPrefix, context.scope()),
            context.random().uuid())),
